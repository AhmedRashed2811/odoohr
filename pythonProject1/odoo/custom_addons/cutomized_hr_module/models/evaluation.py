...
class Evaluation(http.Controller):
    @http.route('/applicants/evaluate/<string:candidates_ids>', type='http', auth='user')
    def download_employees_excel_report(self, candidates_ids):
        candidates_ids = request.env['hr.customized.candidates'].browse(literal_eval(candidates_ids))
        ...
        for candidate in candidates_ids:
            if candidate.cv:
                ...
                if candidate.job_required_id.name not in job_titles:
                    ...
                    return request.redirect('/web#action=566')
        ...
        return request.redirect('/web#action=442')
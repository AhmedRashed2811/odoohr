...
class HrCandidate(models.Model):
    ...
    job_required_id = fields.Many2one('hr.job',string="Job Applied")
    ...
    def candidate_xlsx_report(self):
        return {
            'type': 'ir.actions.act_url',
            'url':f'/candidate_reports/excel/report/{self.env.context.get("active_ids")}',
            'target':'new'
        }
    ...
    def candidate_cv_download(self):
        return {
            'type': 'ir.actions.act_url',
            'url': f'/candidates/cv/download/{self.id}',
            'target': 'self'
        }
    ...
    def evaluate_CVs_AI(self):
        ...
        return {
            'type': 'ir.actions.act_url',
            'url':f'/candidates/evaluate/{self.env.context.get("active_ids")}',
            'target': 'self',
        }
